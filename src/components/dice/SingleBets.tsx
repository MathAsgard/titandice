...
export default function SingleBets() {
  ...
  const [singleBetAmount, setSingleBetAmount] = useState(0);
  const [singleBetPayout, setSingleBetPayout] = useState(0);
  const { credBalance, refetch, loading } = useDice();

  // Payout
  useEffect(() => {
    setSingleBetPayout(Number((payoutRate * singleBetAmount).toFixed(2)));
  }, [singleBetAmount]);
  ...
  const handleChangeInput = (method: string) => {
    if (method === "min") {
      ...
      // Set 1/2 Amount
      setActiveTab("1/2");
      if (singleBetAmount !== 1) {
        setSingleBetAmount(singleBetAmount / 2);
      }
    } else if (method === "x2") {
      // Set 2x Amount
      setActiveTab("x2");
  ...
  const handleSingleBet = async () => {
    if (singleBetAmount === 0 || !account.isConnected) return;
          ...
          >
            ...
            <div className="p-[10px] h-[70px]">
              <input
                ...
                onChange={(e) => setSingleBetAmount(Number(e.target.value))}
              />

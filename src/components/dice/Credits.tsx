...
export default function Credits() {
  ...
  const [titanxAmount, setTitanxAmountToDeposit] = useState<number>(0);
  ...
  const handleChangeTitanx = (rate: number) => {
    ...
    setTitanxAmountToDeposit(amountToBeUpdated);
  };

  const handleDeposit = async () => {
    if (titanxAmount === 0 || !account.isConnected) return;
        ...
        >
          ...
          <div className="p-[10px] ">
            <input
              ...
              min={0}
              value={titanxAmount}
              onChange={(e) => setTitanxAmountToDeposit(Number(e.target.value))}
            />
